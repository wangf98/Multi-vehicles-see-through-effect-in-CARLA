...
def plot_3d_box(img, cam_to_img, ry, dimension, center):
    ...
    for corner in corners:
        ...
        box_3d.append(point)
    bounding_box = np.zeros((8, 2))
    bounding_box[0, :] = np.array([box_3d[0][0], box_3d[0][1]])
    bounding_box[1, :] = np.array([box_3d[1][0], box_3d[1][1]])
    bounding_box[2, :] = np.array([box_3d[5][0], box_3d[5][1]])
    bounding_box[3, :] = np.array([box_3d[4][0], box_3d[4][1]])
    bounding_box[4, :] = np.array([box_3d[2][0], box_3d[2][1]])
    bounding_box[5, :] = np.array([box_3d[3][0], box_3d[3][1]])
    bounding_box[6, :] = np.array([box_3d[7][0], box_3d[7][1]])
    bounding_box[7, :] = np.array([box_3d[6][0], box_3d[6][1]])
    bounding_box = np.matrix(bounding_box)
    # bounding_box = np.concatenate(bounding_box[:, 0], bounding_box[:, 1])
    print(bounding_box)
    return bounding_box
